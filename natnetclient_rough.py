...
class NatNetClient:
    ...
    def __unpack_marker_set(self) -> None:
        n_sets = self.parser.parse('count')
        packet_size = self.parser.unpack('size')

        if self.data_callbacks['markers'] is not None:
            for _ in range(0, n_sets):
                markers = []
                label = self.parser.unpack('label')
                n_markers = self.parser.unpack('count')

                for i in range(n_markers):
                    marker = self.parser.unpack('marker')
                    self.data_callbacks.markers({})
            ...
            self.parser.seek_ahead(packet_size)

    def __unpack_rigid_bodies(self, frame_prefix: int) -> None:
        n_bodies = self.parser.unpack('count')
        packet_size = self.parser.unpack('size')

        if self.data_callbacks['rigid_bodies'] is not None:
            for _ in range(n_bodies):
                rigid_body = self.parser.unpack('rigid_body')
                rigid_body.update(frame_prefix)
                self.data_callbacks.rigid_bodies(rigid_body)
        else:
            self.parser.seek_ahead(packet_size)

    def __unpack_skeletons(self, frame_prefix) -> None:
        n_skeletons = self.parser.unpack('count')
        packet_size = self.parser.unpack('size')

        if self.data_callbacks['skeletons'] is not None:
            for _ in range(n_skeletons):
                skeleton = {
                    'id': self.parser.unpack('id'),
                    'rigid_bodies': self.__unpack_rigid_bodies(frame_prefix),
                }
                self.data_callbacks.skeletons(skeleton)
        else:
            self.parser.seek_ahead(packet_size)

    def __unpack_data(self, data: bytes) -> int:

        self.parser = OptiParse(data=data)

        self.__unpack_marker_set(frame_prefix)
        self.__unpack_rigid_bodies(frame_prefix)
        self.__unpack_skeletons(frame_prefix)
